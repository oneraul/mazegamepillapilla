...
namespace MazeGamePillaPilla
{
    ...
    class VisibilityTest : IIntersectable
    {
        private Vector2 from;
        private Vector2 to;
        private Rectangle aabb;
        private List<Cell> colliders = new List<Cell>();
        private Vector2[] vertices;
        private Vector2[] projectionAxis;

        public VisibilityTest(Vector2 from, Vector2 to)
        {
            this.from = from;
            this.to = to;
        ...
        public void Draw(SpriteBatch spritebatch, Texture2D pixel)
        {
            ...
            foreach (Cell cell in colliders)
            {
                rectangle = new Rectangle(cell.X, cell.Y, Tile.Size, Tile.Size);
                spritebatch.Draw(pixel, rectangle, Color.Blue);
            }

            rectangle = new Rectangle((int)from.X-2, (int)from.Y-2, 4, 4);
            spritebatch.Draw(pixel, rectangle, Color.Red);
            rectangle = new Rectangle((int)to.X - 2, (int)to.Y - 2, 4, 4);
            spritebatch.Draw(pixel, rectangle, Color.Red);
        }
